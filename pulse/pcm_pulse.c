				     ...
				     snd_pcm_uframes_t size)
{
	...
	// }
	syslog(LOG_NOTICE, "[alsa plugins debug] io->rate = %d\n", io->rate);
	if (!pcm->p || !pcm->p->mainloop)
		return -EBADFD;
			   ...
			   snd_pcm_hw_params_t * params)
{
	...
	syslog(LOG_NOTICE, "%s: pcm->ss.rate = %d\n", __func__, pcm->ss.rate);
...
SND_PCM_PLUGIN_DEFINE_FUNC(pulse)
{
	...
	syslog(LOG_NOTICE, "[alsa plugins debug] server = %s fallback_name = %s\n", server, fallback_name);
